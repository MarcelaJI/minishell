...
int	validate_token_syntax(t_data *data)
{
	...
	while (temp)
	{
		if ((is_redirection_token(temp) || temp->token == HERE_DOC)
			&& !temp->next)
			return (error("syntax error near unexpected token: ", "\'newline\'",
					data));
		if (((temp->index == 0 || temp->index == listsize(data->tokens) - 1)
				&& (temp->token == PIPE)) || (temp->token == PIPE && temp->next
				&& temp->next->token == PIPE) || (is_redirection_token(temp)
				&& is_redirection_token(temp->next)) || (temp->token == HERE_DOC
				&& temp->next && temp->next->token == HERE_DOC)
			|| (temp->token == HERE_DOC && temp->next
				&& temp->next->token != LIMITER) || ((is_redirection_token(temp)
					|| temp->token == HERE_DOC) && temp->next
				&& temp->next->token == PIPE))
			return (error("syntax error near unexpected token: ", temp->str,
					data));
...
{
	if ((temp->index == 0) || (temp->prev && temp->prev->token == PIPE)
		|| (temp->prev && !has_command_before_pipe(temp)
			&& (temp->prev->token == FILENAME || temp->prev->token == LIMITER)))
		temp->token = CMD;
	else if (temp->prev && has_command_before_pipe(temp)
		&& !is_redirection_token(temp->prev) && (temp->prev->token == FILENAME
			|| temp->prev->token == CMD || temp->prev->token == LIMITER
			|| temp->prev->token == PARAM))
		temp->token = PARAM;
